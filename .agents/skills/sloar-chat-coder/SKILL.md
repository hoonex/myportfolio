...
For substantial web implementation/review in an unfamiliar repository, or when repeated AI edits make the internal path difficult to trust, read [references/web-architecture-capsule.md](references/web-architecture-capsule.md) before broad source reading. When local execution exists, run `scripts/web-architecture-map.py <repo> --json` to get a bounded topology snapshot first; in connector-only mode, gather the equivalent facts from repository reads. Keep deterministic topology facts separate from semantic ownership: package/config declarations and observed route files may guide where to read next, but route/data/state/component/style/effect ownership is `CONFIRMED` only from task-relevant source evidence. Use the capsule to reduce unnecessary reads, not to replace source truth.
...
For user-facing Android UI claims where layout, contrast, system bars/insets, orientation, reachability, or screenshot evidence can change acceptance, read [references/rendered-ui-evidence.md](references/rendered-ui-evidence.md). Treat automation and rendered visual acceptance as independent gates; bind accepted renders to exact source/artifact identity, inspect the actual rendered evidence when available, and prefer semantic selectors over stale hard-coded coordinates.
