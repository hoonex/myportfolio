...
# Tight pinch: thumb/index must be much closer before the state can latch.
sub_once(
...
# Unlock Web Audio and start the persistent Left/Right synth oscillators in the explicit user gesture.
sub_once(
    r"async function activateAudioFromGesture\(\)\{.*?\nfunction makeVoice",
    """async function activateAudioFromGesture(){if(!ensureAudioGraph())return false;let c=audioEngine.ctx;try{if(c.state!=='running')await c.resume();if(c.state!=='running'){audioEngine.ready=false;resonance.audioError='TAP RESONANCE FOR AUDIO';return false}audioEngine.ready=true;resonance.audioError='';let unlock=c.createOscillator(),ug=c.createGain(),t=c.currentTime;unlock.frequency.setValueAtTime(523.25,t);ug.gain.setValueAtTime(.0001,t);ug.gain.exponentialRampToValueAtTime(.028,t+.012);ug.gain.exponentialRampToValueAtTime(.0001,t+.085);unlock.connect(ug);ug.connect(c.destination);unlock.start(t);unlock.stop(t+.09);makeVoice('Left');makeVoice('Right');return true}catch(e){audioEngine.ready=false;resonance.audioError=String(e?.message||e||'AUDIO BLOCKED').toUpperCase();return false}}
function makeVoice""",
    'audio activation'
)

# Lower camera/decode workload while keeping enough detail for hand and face landmarks.
...
# Keep hand responsive, slow face slightly, and never execute both synchronous ML calls on one video frame.
sub_once(
    r"function loop\(now,meta\)\{.*?\nfunction size",
    """function loop(now,meta){vfc=raf=0;if(!running||route()!==R)return;let v=q('[data-v13-video]');if(v?.readyState>=2){let mediaTime=meta?.mediaTime??v.currentTime,fresh=meta||mediaTime!==lastVideoTime;if(fresh){lastVideoTime=mediaTime;let begin=performance.now(),runs=0,ts=Math.max(0,performance.now()),handDue=handTask&&now-lastH>=85&&!health.hand.recovering,faceDue=faceTask&&now-lastF>=120&&!health.face.recovering;if(handDue){try{hand=handTask.recognizeForVideo(v,ts);lastH=now;runs++;recordSuccess('hand');if(hand?.landmarks?.length)handEmptyRuns=0;else if(++handEmptyRuns>=18&&!health.hand.cpu){handEmptyRuns=0;console.warn('[Vision14.5] hand tracker returned sustained empty results; switching hand task to CPU');recover('hand')}}catch(e){recordFailure('hand',e)}}else if(faceDue){try{face=faceTask.detectForVideo(v,ts);lastF=now;runs++;recordSuccess('face')}catch(e){recordFailure('face',e)}}if(runs)infer=performance.now()-begin;draw(now);frames++;if(now-fpsAt>=700){fps=frames*1000/(now-fpsAt);frames=0;fpsAt=now}}}schedule()}
function size""",
    'inference loop'
)

# Current runtime already has adaptive DPR; tighten its mobile/desktop caps instead of replacing an old fixed cap.
old_cap="cap=coarse?1.12:1.35"
new_cap="cap=coarse?1:1.25"
if old_cap not in s:
    raise SystemExit('adaptive DPR cap missing')
