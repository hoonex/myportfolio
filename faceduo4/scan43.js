function buildUI(){
  ...
  for(let i=0;i<N;i++){
    ...
    const x1=50+44*Math.cos(a-.062),y1=70+63*Math.sin(a-.062),x2=50+44*Math.cos(a+.062),y2=70+63*Math.sin(a+.062);
...
const HQ_REQ={center:7,right:4,left:4,up:4,down:4};
const HQ_COVERAGE=22;
...
function drawLive(raw){
  const c=$('overlay'),r=c.getBoundingClientRect(),d=Math.min(2,devicePixelRatio||1),w=Math.round(r.width*d),h=Math.round(r.height*d);
  if(c.width!==w||c.height!==h){c.width=w;c.height=h}
  const q=c.getContext('2d');q.clearRect(0,0,w,h);if(!raw?.length)return;
  const mp=coverMap($('video').videoWidth,$('video').videoHeight,w,h),P=raw.map(mp);
...
function canonical(raw){
  ...
  const p=raw.map(v=>[v.x*sw,v.y*sh,(v.z||0)*sw]);
  const L=[33,133].map(i=>p[i]),R=[362,263].map(i=>p[i]);
  const lc=[(L[0][0]+L[1][0])/2,(L[0][1]+L[1][1])/2],rc=[(R[0][0]+R[1][0])/2,(R[0][1]+R[1][1])/2];
  const cx=(lc[0]+rc[0])/2,cy=(lc[1]+rc[1])/2,dx=rc[0]-lc[0],dy=rc[1]-lc[1],s=Math.hypot(dx,dy);if(!s)return null;
...
function eyeNormalize(mesh){
  ...
  const lc=[(mesh[33][0]+mesh[133][0])/2,(mesh[33][1]+mesh[133][1])/2,(mesh[33][2]+mesh[133][2])/2],rc=[(mesh[362][0]+mesh[263][0])/2,(mesh[362][1]+mesh[263][1])/2,(mesh[362][2]+mesh[263][2])/2];
  const cx=(lc[0]+rc[0])/2,cy=(lc[1]+rc[1])/2,cz=(lc[2]+rc[2])/2,dx=rc[0]-lc[0],dy=rc[1]-lc[1],s=Math.hypot(dx,dy)||1,ca=dx/s,sa=dy/s;
...
function stabilizeFrame(mesh){
  ...
  const motion=meshError(n,smoothMeshState),alpha=clamp(.36+motion*5.4,.36,.84);
  const out=n.map((p,i)=>[smoothMeshState[i][0]*(1-alpha)+p[0]*alpha,smoothMeshState[i][1]*(1-alpha)+p[1]*alpha,smoothMeshState[i][2]*(1-alpha)+p[2]*alpha]);
...
function frontalize(fr,ref){
  const ang=poseAngles(fr),factors=[.78,1,1.22],ys=ang.yaw?[-1,1]:[1],ps=ang.pitch?[-1,1]:[1];let best=null,bestErr=1e9;
  for(const sy of ys)for(const sp of ps)for(const fy of factors)for(const fp of factors){const m=eyeNormalize(rotate3D(fr.mesh,sy*ang.yaw*fy,sp*ang.pitch*fp)),e=meshError(m,ref);if(e<bestErr){bestErr=e;best=m}}
...
function adjacency(){if(meshAdj)return meshAdj;meshAdj=Array.from({length:478},()=>new Set());const edges=vision?.FaceLandmarker?.FACE_LANDMARKS_TESSELATION||[];for(const e of edges){if(e.start<478&&e.end<478){meshAdj[e.start].add(e.end);meshAdj[e.end].add(e.start)}}return meshAdj}
function smoothSurface(mesh){
  ...
  for(let pass=0;pass<2;pass++){
    ...
    for(let i=0;i<cur.length;i++){
      ...
      const detail=DETAIL_IDS.has(i),contour=CONTOUR_IDS.has(i),nose=NOSE_IDS.has(i);
      const axy=detail ? .012 : (contour ? .018 : .045);
      const azw=detail ? .025 : (nose ? .065 : (contour ? .045 : .095));
...
function fuseMultiView(){
  ...
  const errs=all.map(f=>f.error).filter(Number.isFinite),em=med(errs),emad=med(errs.map(v=>Math.abs(v-em))),limit=Math.max(.07,em+Math.max(.025,emad*2.7));
  const transformed=all.filter(f=>f.bucket==='center'||f.error<=limit);
  ...
  for(let i=0;i<ref.length;i++){
    let x=weightedRobustCoord(transformed,i,0),y=weightedRobustCoord(transformed,i,1),z=weightedRobustCoord(transformed,i,2);
    const rb=DETAIL_IDS.has(i) ? .22 : (CONTOUR_IDS.has(i) ? .07 : .11);
    const zb=NOSE_IDS.has(i) ? .10 : (DETAIL_IDS.has(i) ? .30 : .20);
...
async function loop(t){
  ...
  try{
    ...
    const ts=performance.now(),r=task.detect(inf),elapsed=performance.now()-ts,raw=r.faceLandmarks?.[0];$('perf').textContent=Math.round(elapsed)+'ms';
    inferSamples.push(elapsed);if(inferSamples.length>=8){const avg=inferSamples.reduce((s,x)=>s+x,0)/inferSamples.length;if(avg>115&&inferMax>760)inferMax-=60;else if(avg<58&&inferMax<960)inferMax+=30;inferSamples=[]}
    if(!raw?.length){$('hint').textContent='얼굴을 화면 안에 맞춰 주세요';return}
    drawLive(raw);const base=canonical(raw),po=pose(raw);if(!base)return;const stable=stabilizeFrame(base),m=stable.mesh,fr={mesh:m,feat:feature(m),pose:po,stability:stable.stability};
    if(phaseName==='front'){
      const frontOK=Math.abs(po.yaw)<.06;
      if(frontOK){const dt=Math.min(140,t-lastLoop);hold+=dt;centerFrames.push(fr);if(centerFrames.length>24)centerFrames.shift();if(!bestFront||Math.abs(po.yaw)<Math.abs(bestFront.pose.yaw))bestFront=fr;$('percent').textContent=Math.min(20,Math.floor(hold/720*20))+'%';if(hold>=720&&centerFrames.length>=HQ_REQ.center){basePitch=med(centerFrames.map(x=>x.pose.pitch));phaseName='orbit';$('phase').textContent='고정밀 다각도 측정';$('capTitle').textContent='고개를 천천히 움직여 주세요';$('hint').textContent='원을 따라 좌·우·위·아래를 천천히 훑어주세요'}}else{hold=Math.max(0,hold-80);$('hint').textContent='정면을 바라봐 주세요'}
    }else if(phaseName==='orbit'){
      const dy=po.pitch-basePitch,x=clamp(-po.yaw/.16,-1.25,1.25),y=clamp(dy/.09,-1.25,1.25),rad=Math.hypot(x,y);currentBin=-1;
      if(rad>.36&&rad<1.36){const a=(Math.atan2(y,x)+Math.PI*2)%(Math.PI*2),bin=Math.round(a/(Math.PI*2)*N)%N,currentQuality=clamp(fr.stability,.58,1),bucket=bucketName(po.yaw,dy);currentBin=bin;bins[bin]++;binQuality[bin]+=currentQuality;orbitFrames.push({...fr,bucket,bin,quality:currentQuality});if(orbitFrames.length>280)orbitFrames.shift()}
      updateProgress()
    }
    lastLoop=t
...
function makePayload(){
  const fused=fuseMultiView(),cm=fused.mesh;if(!cm)throw Error('다각도 mesh fusion에 실패했습니다.');
  const groups={center:centerFrames,right:orbitFrames.filter(f=>f.bucket==='right'),left:orbitFrames.filter(f=>f.bucket==='left'),up:orbitFrames.filter(f=>f.bucket==='up'),down:orbitFrames.filter(f=>f.bucket==='down')},views={};for(const k of KEYS)views[k]=medianVec(groups[k]);
