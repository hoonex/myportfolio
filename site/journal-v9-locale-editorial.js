...
(() => {
  const COPY = {
    ...
    en: {
      ...
      controls: 'Optical parameters',
      reset: 'Reset',
      config: 'Current shader configuration',
      notes: [
  ...
  const CONTROL_LABELS = {
    ko: {
      refraction:'굴절', blurAmount:'Blur', chromAberration:'색수차', specular:'Specular', fresnel:'Fresnel',
      edgeHighlight:'경계광', zRadius:'Bevel 깊이', cornerRadius:'모서리', saturation:'채도', brightness:'밝기', width:'너비', height:'높이'
    },
    en: {
      refraction:'Refraction', blurAmount:'Blur', chromAberration:'Chromatic', specular:'Specular', fresnel:'Fresnel',
      edgeHighlight:'Edge light', zRadius:'Bevel depth', cornerRadius:'Corner', saturation:'Saturation', brightness:'Brightness', width:'Width', height:'Height'
    },
    ja: {
      refraction:'屈折', blurAmount:'ぼかし', chromAberration:'色収差', specular:'鏡面反射', fresnel:'Fresnel',
      edgeHighlight:'縁の光', zRadius:'ベベル深度', cornerRadius:'角丸', saturation:'彩度', brightness:'明るさ', width:'幅', height:'高さ'
    }
  };
  ...
  function applyRefractionEdition() {
    ...
    const intro = root.querySelector('.refraction-header > p');
    if (intro) intro.textContent = copy.intro;
    const drag = root.querySelector('.refraction-drag-note');
    if (drag) drag.textContent = copy.drag;
    const controlsTitle = root.querySelector('.ref-controls-head strong');
    if (controlsTitle) controlsTitle.textContent = copy.controls;
    const reset = root.querySelector('#refReset');
    if (reset) reset.textContent = copy.reset;
    const config = root.querySelector('.ref-config > span');
    if (config) config.textContent = copy.config;

    Object.entries(CONTROL_LABELS[lang]).forEach(([id, label]) => {
      const input = root.querySelector(`#ref-${id}`);
      const row = input?.closest('.ref-control');
      const node = row?.querySelector(':scope > span');
      if (node) node.textContent = label;
    });

    root.querySelectorAll('.refraction-explainer > article').forEach((article, index) => {
      const [title, body] = copy.notes[index] || [];
      if (!title) return;
  ...
  function applyGlassArticleEdition() {
    ...
    const heading = correction.querySelector('h2');
    const paragraph = correction.querySelector(':scope > p');
    if (heading) heading.textContent = copy.articleTitle;
    if (paragraph) paragraph.textContent = copy.articleBody;
    correction.querySelectorAll('.refraction-architecture span').forEach((node, index) => {
      if (copy.architecture[index]) node.textContent = copy.architecture[index];
    });
  }
