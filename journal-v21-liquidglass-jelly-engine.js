/* Journal v21: LiquidGlass-native soft-body deformation engine.
   All jelly deformation is applied inside the same SDF/height field that drives
   refraction. No decorative DOM dent/scale layer is used. */
(() => {
  const proto = globalThis.WebGLRenderingContext?.prototype;
  if (!proto || proto.__hjLiquidV21Patch) return;
  Object.defineProperty(proto, '__hjLiquidV21Patch', { value: true, configurable: false });
  ...
  proto.drawArrays = function(...args) {
    try {
      ...
      if (loc) {
        ...
        if (loc.jellyAngle) this.uniform1f(loc.jellyAngle, jellyOn ? clamp(finite(j.angle),-.24,.24) : 0);
        if (loc.jellyStretch) this.uniform2f(loc.jellyStretch,
          jellyOn ? clamp(finite(j.stretchX,1),.58,1.70) : 1,
          jellyOn ? clamp(finite(j.stretchY,1),.58,1.70) : 1
        );
        ...
        if (loc.jellyShear) this.uniform2f(loc.jellyShear,
          jellyOn ? clamp(finite(j.shearX),-.35,.35) : 0,
          jellyOn ? clamp(finite(j.shearY),-.35,.35) : 0
        );
        if (loc.jellyWave) this.uniform4f(loc.jellyWave,
          jellyOn ? clamp(finite(j.waveX),0,14) : 0,
          jellyOn ? clamp(finite(j.waveY),0,14) : 0,
        ...
        for (let i=0;i<4;i++) {
          ...
          if (loc.contacts[i]) this.uniform4f(loc.contacts[i],
            ...
            jellyOn ? clamp(finite(c.depth),0,1.3) : 0,
            clamp(finite(c.radius,.28),.06,.86)
          );
          if (loc.contactAxes[i]) this.uniform2f(loc.contactAxes[i],
            clamp(finite(c.axisX,1),.24,2.1), clamp(finite(c.axisY,1),.24,2.1)
          );
        }
      }
    } catch (error) {
      console.warn('HJ LiquidGlass v21 uniform update failed:', error);
    }
